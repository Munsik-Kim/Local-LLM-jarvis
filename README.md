...
죄송합니다. 채팅창의 코드 블록 안에 또 코드 블록(```)이 들어가면 화면이 깨져 보이는 경우가 있습니다.

이번에는 복사했을 때 깨지지 않도록, 내부의 코드 블록 표시를 유지한 상태로 안전하게 드리겠습니다. 아래 내용을 처음부터 끝까지 복사해서 README.md 파일에 덮어씌워 주세요.
...
git clone https://github.com/Munsik-Kim/Local-LLM-jarvis.git
...
위 내용을 복사해서 저장한 뒤, 터미널에서 아래 명령어로 마무리해 주세요.
